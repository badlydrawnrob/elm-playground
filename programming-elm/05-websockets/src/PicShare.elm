...
{-| Communicating with the servers
    ...
    2. We're still using `List.map`, but our `Maybe.map` is now "lifting" a
      ...
      return a `Feed` (which is a `List Photo`)
        - It's also possible to use `List.filter` but that'd only filter a single
          `Photo` (with `ID`) so we'd have to rejoin it to the main `List Photo`
          (our `Feed`): @ https://tinyurl.com/elm-playground-4d7819c
...
import Http
import WebSocket as WS exposing (listen, receive)
