	...
	function (m, string) {
		var check = function () {
			if (!m.b) {
				...
				if (!m.b.b) {
					var s = m.a;
					return $elm$core$String$length(s) <= 2;
				} else {
					return false;
				}
			}
...
var $author$project$Result$FieldError$convertError = function (fe) {
	switch (fe.$) {
		case 'YayNoErrors':
			return '';
		case 'EmptyOrNotString':
			return 'Please enter a float string';
		case 'NotProperFloat':
			return 'This isn\'t a proper float';
		case 'NumbersTooHigh':
			return 'Please keep numbers in range';
		default:
					...
					[
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$text(
								$elm$core$String$fromFloat(model.savedInput))
							]))
					]))
			]));
};
